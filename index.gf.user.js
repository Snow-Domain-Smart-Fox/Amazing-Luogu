...
// @connect      www.cpoauth.com
// @connect      unpkg.com
// @connect      fastly.jsdelivr.net
// @connect      *
// @require      https://fastly.jsdelivr.net/npm/sweetalert2@11.26.17/dist/sweetalert2.min.js?version=1.0.1
// @require      https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.10.0/highlight.min.js
// @require      https://fastly.jsdelivr.net/npm/marked@4.0.0/marked.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/dompurify@3.3.1/dist/purify.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/katex@0.16.27/dist/katex.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/katex@0.16.27/dist/contrib/auto-render.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/@fortawesome/fontawesome-free@7.1.0/js/all.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/jquery@3.7.1/dist/jquery.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/izitoast@1.4.0/dist/js/iziToast.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/clipboard@2.0.11/dist/clipboard.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/vue@2.6.11/dist/vue.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/mark.js@8.11.1/dist/mark.min.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/fuse.js@7.2.0/dist/fuse.js?version=1.0.1
// @require      https://fastly.jsdelivr.net/npm/@waline/client@3.15.2/dist/waline.umd.js?version=1.0.8
// @require      https://fastly.jsdelivr.net/npm/@trim21/gm-fetch@0.3.0/dist/gm_fetch.js?version=1.2.5
// @require      https://fastly.jsdelivr.net/npm/@tensorflow/tfjs@4.22.0/dist/tf.min.js?version=1.3.3
// @resource     iziToastCSS https://fastly.jsdelivr.net/npm/izitoast@1.4.0/dist/css/iziToast.min.css?version=1.0.1
// @resource     icomoonCSS https://fastly.jsdelivr.net/gh/marcelodolza/iziToast@709dce63e17fea725c215758c31d3a0dcbabe0b3/docs/css/icomoon.css?version=1.0.1
// @resource     hljs https://fastly.jsdelivr.net/npm/highlight.js@11/styles/github.min.css
// @resource     swal https://fastly.jsdelivr.net/npm/sweetalert2@11.26.17/dist/sweetalert2.min.css?version=1.0.1
// @resource     animate https://fastly.jsdelivr.net/npm/animate.css@4.1.1/animate.min.css?version=1.0.1
// @resource     walineCSS https://fastly.jsdelivr.net/npm/@waline/client@3.15.2/dist/waline.css?version=1.0.8
...
let MODEL_TOPOLOGY = null;
let WEIGHT_SPECS = null;
let WEIGHTS_BASE64 = null;
let isnew = null;
	...
	if (Notificationaaaaaaa) {
		...
		sortedResult.forEach((notification) => {
			...
			node.onclick = function () {
				...
				console.debug(document.getElementById("aml-modal-header"));
				...
				console.debug(document.getElementById("aml-modal-body"));
		...
		(function ($, undefined) {
			...
			const features = [
				...
				},
				{
					...
					desc: "在讨论页面按下 Ctrl+S 来保存当前查看的讨论到 luogu.ac.cn，API 来自 luogu.ac.cn",
					tag: "功能",
					status: "stable",
				},
				{
				...
				},
				{
					...
					label: "讨论区复制",
					desc: "在讨论区帖子中添加复制 Markdown 按钮和引用按钮，可能会导致部分洛谷特殊 Markdown 失效",
					tag: "功能",
					status: "stable",
				},
				{
				...
				},
				{
					key: "codeFolding",
					label: "代码折叠、扫描与复制",
					desc: "自动折叠过长的代码块以便阅读并自动识别危险代码，新老前端皆可复制，并去除洛谷原生按钮",
					tag: "功能",
					status: "stable",
				},
			];
	...
	<div class="welcomeContainer">
		<div class="welcomeRight">
				...
				<div class="aml-tab-content-container" role="tabpanel">
					...
					<div class="aml-tab-content" data-tab-pane="update" role="tabpanel" id="update-panel">
						...
						<div class="announcement-section aml-home-card" style="margin-top: 20px;">
							<h5><i class="fas fa-rocket"></i> v1.2.5 - 2026/7/14</h5>
							<div class="announcement-content">
								<p><strong>注册流程升级，接入 CP OAuth 统一身份认证平台！</strong></p>
							</div>
							<div class="features-section aml-home-card" style="margin-top: 10px">
								<h5><i class="fas fa-key"></i> 安全升级</h5>
								<ul class="announcement-content" style="list-style: none; padding-left: 0;">
									<li style="margin-bottom: 8px;"><i class="fas fa-check-circle" style="color: #6366f1; margin-right: 8px;"></i>使用 CP OAuth 授权码模式替代简介验证，无需修改个人简介</li>
									<li style="margin-bottom: 8px;"><i class="fas fa-check-circle" style="color: #6366f1; margin-right: 8px;"></i>实现 PKCE 安全流程，防止授权码被窃取</li>
									<li style="margin-bottom: 8px;"><i class="fas fa-check-circle" style="color: #6366f1; margin-right: 8px;"></i>弹窗授权方式，用户体验更友好</li>
								</ul>
							</div>
			...
			<script>
				...
				if(typeof exploreScripts=='undefined'){
					exploreScripts = [
						...
						{ name: '洛谷档案馆', icon: 'fa-regular fa-floppy-disk', type: 'website', desc: '强大的洛谷保存站。', link: 'https://luogu.ac.cn/' },
				...
				(function initAfterRender() {
					setTimeout(() => {
						...
						console.debug('Current user ID:', getCurrentUserId());
						(function initThanks() {
							...
							addManagedEventListener(thanksBtn, 'click', function renderThanks() {
								...
								contributors.forEach(contrib => {
									...
									console.debug('Checking follow status for user', contrib.id, 'currentUid:', getCurrentUserId(), 'getCurrentUserId', getCurrentUserId);
				...
				console.debug(my_info);
			}
			let homeDataMidnightTimer = null;
			async function fetchHomeData() {
				...
				function getNextMidnightTimestamp(now) {
					const date = new Date(now);
					date.setHours(24, 0, 0, 0);
					return date.getTime();
				}
				function getCache(key, expireKey) {
					try {
						const expireAt = Number(GM_getValue(expireKey, 0));
				...
				function setCache(key, expireKey, data) {
					try {
						GM_setValue(key, data);
						GM_setValue(expireKey, getNextMidnightTimestamp(Date.now()));
					} catch (e) { }
				}
				...
				if (homeDataMidnightTimer === null) {
					const now = Date.now();
					homeDataMidnightTimer = setTimeout(() => {
						homeDataMidnightTimer = null;
						if (document.getElementsByClassName("aml-yiyan")[0]) {
							fetchHomeData();
						}
					}, getNextMidnightTimestamp(now) - now);
				}
			...
			async function getusername(uid) {
				...
				console.debug(`https://www.luogu.com.cn/api/user/search?keyword=${uid}`);
		...
		<div class="aml-settings-section aml-home-card">
			...
			<div id="aml-aiproblemanalysis-section" style="display: ${currentSettings.aiProblemAnalysisEnabled ? "block" : "none"};">
				...
				<div class="aml-tip">注意：仅支持 OpenAI Chat Completions API 格式，AI 不会输出代码，只会提供分析和思路指导。</div>
			</div>
					...
					if (checkbox) {
						checkbox.onchange = (e) => {
							...
							if (feature.key === "slogenTimeEnabled") {
								currentAMLSettings.slogenTimeEnabled = newValue;
					...
					if (settings.lsawArticleDisplay != false) {
						GM_xmlhttpRequest({
							...
							onload: function (response) {
								...
								console.debug(json);
								if (json.total != 0) {
									console.debug('文章数量:', json.hits.length);
    ...
    `;
			...
			if (
				...
				function wrapPreInFoldable(preElement) {
					if (preElement.closest(".aml-code-foldable-wrapper")) return;
					if (preElement.classList.contains("aml-code-processed")) return;
					...
					const nativeCopy = codeContainer ? codeContainer.querySelector(".copy-button") : preElement.parentNode?.querySelector(".copy-button");
					if (nativeCopy) nativeCopy.remove();
					...
					if (codeElement && codeElement.className) {
						...
						if (match && match[1]) language = match[1].toUpperCase();
					}
					const header = document.createElement("div");
					header.className = "aml-code-fold-header";
					header.innerHTML = `
				        <span class="aml-code-title">${language}</span>
				        <span class="aml-copy-code-btn" title="复制代码"><i class="fas fa-copy"></i></span>
				        <div class="aml-toggle-icon-wrapper"><i class="fas fa-chevron-down aml-toggle-icon"></i></div>
				    `;
					const copyBtn = header.querySelector('.aml-copy-code-btn');
					if (copyBtn) {
						addManagedEventListener(copyBtn, 'click', function (e) {
							e.stopPropagation();
					...
					const code = preElement.querySelector("code");
					const shouldFold = code && code.textContent.length > 1500;
					if (shouldFold) {
						content.classList.add("aml-collapsed");
						header.querySelector(".aml-toggle-icon-wrapper").classList.add("aml-folded-icon");
					...
					addManagedEventListener(header, "click", (event) => {
						if (event.target.closest('.aml-copy-code-btn')) return;
						...
						const targetH = Math.min(realHeight, 800);
						if (isFolded) {
							content.classList.remove("aml-collapsed", "aml-expanded");
							...
							wrapper.classList.add("aml-unfolded");
							header.querySelector(".aml-toggle-icon-wrapper").classList.remove("aml-folded-icon");
						} else {
							content.style.maxHeight = targetH + 'px';
							void content.offsetHeight;
							content.style.maxHeight = '0px';
				...
				function unwrapAllCodeBlocks() {
					document.querySelectorAll(".aml-code-foldable-wrapper").forEach((wrapper) => {
						const content = wrapper.querySelector(".aml-code-content");
						const inner = content ? (content.querySelector("pre") || content.querySelector(".code-container") || content.firstElementChild) : null;
						if (inner && wrapper.parentNode) {
							inner.classList.remove("aml-code-processed");
...
.aml-copy-code-btn:hover {
    color: #6366f1;
}
.aml-copy-code-btn.copied {
    color: #52c41a;
}
						...
						},
						{
							...
							getContent: async () => {
								return (
									JSON.parse(
										document.getElementById("lentille-context").innerHTML,
									).data.training.description || "复制失败"
								);
							},
						},
						{
    ...
    background: rgba(var(--l-button--real-color), 0) none;`;
					...
					if (isQuote) {
						addManagedEventListener(button, "click", (e) => {
							...
							if (typeof GM_setClipboard === "function") {
								...
								navigator.clipboard.writeText(formatAsQuote(content)).catch(async (err) => {
									await Swal.fire({
										title: "错误",
										text: "复制失败",
										icon: "error",
						...
						addManagedEventListener(button, "click", (e) => {
							...
							if (typeof GM_setClipboard === "function") {
								...
								navigator.clipboard.writeText(copyContent).catch(async (err) => {
									await Swal.fire({
										title: "错误",
										text: "复制失败",
										icon: "error",
				...
				function processMainContent() {
					waitForElement(".lfe-marked-wrap.marked", async (markedWrap) => {
				...
				function fixReplyTexts() {
					document.querySelectorAll(".comment").forEach((comment, index) => {
						...
						try {
							markedInner.innerHTML = DOMPurify.sanitize(marked.parse(expectedContent));
							if (typeof renderMathInElement === "function") {
								renderMathInElement(markedInner, {
									delimiters: [
										{ left: "$$", right: "$$", display: true },
										{ left: "$", right: "$", display: false },
									],
				...
				const reprocessAfterNavigation = () => {
				...
				addManagedEventListener(window, "popstate", () => {
					setTimeout(checkUrlChange, 0);
				});
				addManagedEventListener(document, "click", (e) => {
					if (e.target.closest && e.target.closest("a")) {
						if (e.target.closest("a.aml-copy-md-btn")) return;
						cleanupForNavigation();
					}
				}, true);
			...
			if (
				...
				try {
					async function addWalineComment() {
						console.debug("[Waline DEBUG] 开始执行 addWalineComment");
						if (document.getElementById("waline")) {
							console.debug("[Waline DEBUG] waline 元素已存在，跳过");
							await Swal.fire({
								title: "提示",
								text: "Waline 评论已加载",
								icon: "info",
						...
						if (!mainElement) {
							console.debug("[Waline DEBUG] 未找到 main 元素");
							await Swal.fire({
								title: "错误",
								text: "未找到 main 元素",
								icon: "error",
						...
						console.debug("[Waline DEBUG] 找到 main 元素，子元素数量:", mainElement.children.length);
						...
						if (!targetElement) {
							console.debug("[Waline DEBUG] 未找到 targetElement");
							await Swal.fire({
								title: "错误",
								text: "未找到目标元素",
								icon: "error",
						...
						console.debug("[Waline DEBUG] 找到 targetElement:", targetElement);
						const walineDiv = document.createElement("div");
						walineDiv.id = "waline";
						targetElement.parentNode.insertBefore(walineDiv, targetElement.nextSibling);
						console.debug("[Waline DEBUG] 创建 waline div 成功");
						console.debug("[Waline DEBUG] Waline 对象:", window.Waline);
						if (window.Waline) {
							console.debug("[Waline DEBUG] 初始化 Waline");
							window.Waline.init({
								el: "#waline",
							...
							console.debug("[Waline DEBUG] window.Waline 未定义");
							await Swal.fire({
								title: "错误",
								text: "Waline 模块未正确加载",
								icon: "error",
			...
			if (currentAMLSettings.acceptedProblemCmpEnabled) {
				try {
					...
					console.debug(CACHE, TIME, CACHE_TYPE);
					if (
						TIME < Date.now() - 1000 * 60 * 60 * 24 ||
						CACHE_TYPE != "1.3.6-Self-Update"
					) {
						console.debug("ACPCACHE过期，开始更新");
						(async function () {
							console.debug("更新ACPCACHE");
							...
							GM_setValue("ACPCACHE_TYPE", "1.3.6-Self-Update");
							console.debug("更新ACPCACHE完成");
							console.debug(newCache);
						})();
					}
				} catch (e) {
			...
			if (
				...
				location.pathname.split("/")[2] != getCurrentUserId()
			) {
				try {
				...
				try {
					(async function () {
						...
						async function loadCaptcha() {
							let code = GM_getValue("OCRCaptchaCode");
							if (!code) {
								let res = await new Promise((resolve, reject) => {
									GM_xmlhttpRequest({
										method: "GET",
										url: "https://cdn.amlg.top/amlg/OCRCaptcha.js",
										timeout: 10000,
										onload: resolve,
										onerror: reject,
								...
								if (res.status < 200 || res.status >= 300) {
									await Swal.fire({ title: "错误", text: "自动填充字母识别验证码模块加载失败", icon: "error", confirmButtonText: "确定" });
									return false;
								}
								code = res.responseText;
								try { GM_setValue("OCRCaptchaCode", code); } catch (e) { console.warn("缓存写入失败(可能超存储上限)", e); }
							}
							try {
								const get = new Function(code + "; return { MODEL_TOPOLOGY, WEIGHT_SPECS, WEIGHTS_BASE64 };");
								const data = get();
								MODEL_TOPOLOGY = data.MODEL_TOPOLOGY;
								WEIGHT_SPECS = data.WEIGHT_SPECS;
								WEIGHTS_BASE64 = data.WEIGHTS_BASE64;
								console.debug("Load captcha code success");
								return true;
							} catch (e) {
								console.debug("Load captcha code failed", e);
								GM_setValue("OCRCaptchaCode", "");
								await Swal.fire({ title: "错误", text: "自动填充字母识别验证码模块加载失败", icon: "error", confirmButtonText: "确定" });
								return false;
							}
						}
						...
						async function ensureModel() {
							...
							const topo = JSON.parse(JSON.stringify(MODEL_TOPOLOGY));
							const topoConfig = topo.model_config.config;
							if (topo.model_config.class_name === "Functional") topo.model_config.class_name = "Model";
							const topoLayers = topoConfig.layers;
							for (const l of topoLayers) {
								if (l.class_name === "InputLayer" && l.config.batch_shape && !l.config.batchInputShape) {
								...
								if (l.config && l.config.dtype && typeof l.config.dtype === "object") {
									var dn = l.config.dtype.config && l.config.dtype.config.name;
									l.config.dtype = dn === "mixed_float16" ? "float32" : (dn || "float32");
								}
							}
							if (topoConfig.input_layers && topoConfig.input_layers.length > 0 && !Array.isArray(topoConfig.input_layers[0])) {
								topoConfig.input_layers = [topoConfig.input_layers];
							}
							if (topoConfig.output_layers && topoConfig.output_layers.length > 0 && !Array.isArray(topoConfig.output_layers[0])) {
								topoConfig.output_layers = [topoConfig.output_layers];
							}
						...
						const CONFIGS = [
							{
								...
								input: "input.lform-size-middle[placeholder*='图形验证码']",
							},
							{
						...
						function startWatch() {
							...
							sigCanvas.width = img.naturalWidth;
							sigCanvas.height = img.naturalHeight;
							const sigCtx = sigCanvas.getContext("2d");
							sigCtx.drawImage(img, 0, 0);
							const sig = sigCanvas.toDataURL("image/jpeg").split(",")[1];
							if (sig !== lastSig) {
								lastSig = sig;
						...
						function recognizeAndFill(img, input) {
							ensureModel().then((model) => {
								return tf.tidy(() => {
									const canvas = document.createElement("canvas");
									canvas.width = 90;
									canvas.height = 35;
									const ctx = canvas.getContext("2d");
									ctx.drawImage(img, 0, 0, 90, 35);
									const inputTensor = tf.browser.fromPixels(canvas).div(255).expandDims(0);
									const logits = model.predict(inputTensor);
									const chars = [];
									for (let i = 0; i < 4; i++) {
										const idx = logits.slice([0, i, 0], [1, 1, 35]).argMax(-1).dataSync()[0];
								...
								code = (code || "").trim();
								if (!code) {
									console.debug("⚠️ 不合规，刷新 | 结果：", code);
									return;
								}
								const isLegalCode = /^[0-9a-zA-Z]{4}$/.test(code);
								if (!isLegalCode) {
									console.debug("⚠️ 不合规，刷新 | 结果：", code);
									input.value = "";
									lastSig = "";
								...
								console.debug("✅ 识别填充成功：", code);
							}).catch((e) => {
								console.error("❌ 识别失败", e);
							});
						}
						ensureModel().catch((e) => {
							console.error("❌ OCR 模型加载失败", e);
						});
					})();
				} catch (e) {
					console.debug(e);
							...
							try {
								...
								if (response.ok) {
									...
									if (response2.ok) {
										...
										await Swal.fire("创建失败", "请稍后重试", "error");
									}
								} else {
									await Swal.fire("创建失败", "请稍后重试", "error");
								}
...
.cmd-output{color:#aaa}
    ...
    `;
					...
					if (document.querySelector(".tasklist-item")) {
						...
						for (let i = 0; i < tasklist.length; i++) {
							...
							let accepted =
								...
								"lg-fg-green";
							if (accepted) {
								console.debug("Task " + pid + " accepted.");
								task.remove();
								pidList.remove(pid);
							}
						}
				...
				try {
					...
					$(document).on("keydown", async function (e) {
						if ((event.ctrlKey || event.metaKey) && e.key === "s") {
							e.preventDefault();
							try {
								const url = "https://luogu.ac.cn/api/v1/save";
								GM_xmlhttpRequest({
									method: "POST",
									url: url,
									headers: {
			...
			if (
				...
				try {
					console.debug("Codeforces 原题难度显示功能触发");
					console.debug(
			...
			if (typeof addManagedEventListener !== 'function') {
				window.addManagedEventListener = function (element, event, handler) {
			...
			const AI_CACHE_KEY = 'aml_ai_analysis_cache';
			function getAICache() {
				try { return JSON.parse(localStorage.getItem(AI_CACHE_KEY) || '{}'); } catch { return {}; }
			}
			function setAICache(path, content) {
				const cache = getAICache();
				cache[path] = { content, timestamp: Date.now() };
				localStorage.setItem(AI_CACHE_KEY, JSON.stringify(cache));
			}
			function getAICacheForCurrentPath() {
				const path = location.pathname;
				const cache = getAICache();
				return cache[path] || null;
			}
			function clearAICacheForCurrentPath() {
				const path = location.pathname;
				const cache = getAICache();
				delete cache[path];
				localStorage.setItem(AI_CACHE_KEY, JSON.stringify(cache));
			}
			const shouldShowAIAnalysisButton = () => {
				const hasContestId = !!(new URLSearchParams(location.href.split('?')[1])).get('contestId');
				return (
			...
			function getProblemText() {
				...
				if (!lentilleContext) return null;
				try {
					const problemData = JSON.parse(lentilleContext.innerHTML).data.problem;
					return `题目编号：${problemData.pid || ""}\n题目名称：${problemData.name || ""}\n时间限制：${problemData.limits.time || NaN}ms\n内存限制：${(problemData.limits.memory || NaN) / 1024}MB\n题目（JSON格式）：${JSON.stringify(problemData.content) || ""}\n样例（JSON List格式）：${JSON.stringify(problemData.samples) || ""}`;
				} catch (e) {
					console.error('[AMLG] 解析题目数据失败:', e);
					return null;
				}
			}
			function getSystemPrompt() {
				return `你是一名资深算法竞赛教练，擅长分析洛谷（Luogu）平台上的编程题目。请对用户提供的题目进行系统化、结构化分析，帮助学习者理解题目本质、推导解法并识别潜在陷阱。

【硬性约束】
1. 输出语言：始终使用中文。
2. 禁止代码：严禁输出任何形式的代码，包括具体语言代码、伪代码、代码块、语法示例、编程语句；所有解答只能用自然语言、数学公式或逻辑表达式描述。
3. LaTeX 规范：行内公式用 $...$，独立公式用 $$...$$；严禁使用 \\( \\) 或 \\[ \\] 等其他定界符。
4. 推导自洽：所有数学关系（状态转移方程、递推式、边权计算、公式变形）必须给出推导过程并自检符号与逻辑正确性。
5. 算法对应：复杂度分析必须与你所选用的算法严格对应，不要分析与算法无关的内容。

【输出格式】
严格按以下顺序与标题输出，不得增删一级标题：

---
**题目类型**：<算法类别，如"动态规划/背包"、"图论/最短路"、"贪心"、"数论"、"模拟"等，可细分；若跨类请标注主次>
...
**题意简化**：<一两句话概括求解目标，剥离故事背景>

**数据范围分析**：<结合时间/内存限制与数据规模，反推所需算法复杂度上界，说明哪些算法因此被排除>

**算法选择**：<先列出 1~2 个可行算法并比较优劣，再明确最终选用算法及选用理由；若仅一种合理算法可直接说明>

**数学建模与逻辑推导**：
- 变量/状态定义：<自然语言描述>
- 模型构建：<将题目条件转化为数学或逻辑模型的过程>
- 核心关系式：<转移方程、递推式或判定条件的自然语言+数学公式描述，并附推导与正确性验证>
...
- <边界条件/特殊情况>
- <常见误区或易忽略细节>
- <若涉及负权、负环、自环、重边、离散化、精度等特殊情形，明确说明检测与处理方式>

**复杂度分析**：
- 时间复杂度：<针对所选算法给出表达式，如 O(N+M)、O(NM)、O((N+M)logN)，并解释各项来源>
- 空间复杂度：<表达式及来源说明>

**延伸思考**：<可选：变种、优化空间、同类经典题或降维方法>
---

【质量要求】
- 分析严谨、通俗易懂，逻辑链完整。
- 若题目存在多种合理解法，需在"算法选择"中比较后再进入推导。
- 若题目数据范围不明确，可基于常规假设分析并注明假设。`;
			}
			function getUserPrompt(problemText) {
				return `请分析以下洛谷题目，并严格按照系统提示的格式输出完整分析。
【特别注意】
- 本次回答中严禁输出任何代码实现（包括伪代码），所有内容必须是自然语言和数学公式描述。
- 请务必验证你的数学推导是否正确，特别是涉及边权、递推公式等部分。
- 若分析到图论最短路且边权可能为负，请指明适用算法及负环处理策略。
- 复杂度分析必须与所选算法对应。
- 所有LaTeX语句必须使用$的形式。
--- 题目原文 ---
${problemText}
...
请开始你的分析。`;
			...
			function setupLongPress(btn, callback, seconds = 5) {
				let rafId = null;
				let progress = null;
				...
				const start = (e) => {
					...
					animationToken++;
					const oldProgress = btn.querySelector('.ai-action-progress');
					if (oldProgress) oldProgress.remove();
					progress = document.createElement('span');
					progress.className = 'ai-action-progress';
					progress.style.width = '0%';
					btn.appendChild(progress);
					let wrapper = btn.querySelector('.aml-confirm-btn-text');
					if (!wrapper) {
						wrapper = document.createElement('span');
						wrapper.className = 'aml-confirm-btn-text';
						const children = Array.from(btn.childNodes);
						children.forEach(child => {
							if (child.nodeType === 3) wrapper.appendChild(child);
						});
						if (!wrapper.textContent) wrapper.textContent = btn.textContent || '确认';
					...
					function animate() {
						...
						const remain = Math.max(total - Math.floor(elapsed), 0);
						progress.style.width = percent + '%';
						if (remain !== lastSecond) {
							lastSecond = remain;
							if (remain > 0) {
								textWrapper.textContent = `${btn.dataset.originalText || '确认'}(${remain}秒)`;
							} else {
								textWrapper.textContent = btn.dataset.originalText || '确认';
				...
				const updateDisplay = () => {
					if (!textWrapper || !progress) return;
					const elapsed = (Date.now() - startTime) / 1000;
					const remain = Math.max(total - Math.floor(elapsed), 0);
					const percent = Math.min((elapsed / total) * 100, 100);
					progress.style.width = percent + '%';
			...
			function showModal() {
				const modal = document.getElementById('aml-ai-modal');
			...
			function renderConfirmUI() {
				const content = document.getElementById('aml-ai-content');
				const actions = document.getElementById('aml-ai-actions');
				content.innerHTML = `
			        <div style="padding:20px 0;line-height:1.8;color:#334155;font-size:14px;">
			            <p>确认要查看吗？建议先多思考一会儿哦。</p>
			            <p>AI 不会输出代码，只会提供分析和思路指导。</p>
			            <p>所有内容均为人工智能生成，不作任何保证。</p>
			        </div>
			    `;
				actions.innerHTML = `
			        <button id="aml-confirm-btn" class="lform-size-middle button-transparent aml-confirm-btn-wrap" type="button" data-v-505b6a97>查看！</button>
			    `;
				const confirmBtn = document.getElementById('aml-confirm-btn');
				setupLongPress(confirmBtn, () => {
					startAnalysis();
				}, 5);
			}
			function renderLoadingUI() {
				...
				const actions = document.getElementById('aml-ai-actions');
				content.innerHTML = `
			        <div style="display:flex;gap:12px;color:#64748b;min-height:200px;">
			            <span class="aml-spinner"></span> 正在分析中...
			        </div>
			    `;
				actions.innerHTML = '';
			}
			function renderResultUI(contentText) {
				const content = document.getElementById('aml-ai-content');
				const actions = document.getElementById('aml-ai-actions');
				try {
					const html = marked.parse(contentText, { breaks: true });
					...
					disclaimer.style.cssText = 'font-style:italic;color:#888;margin-top:20px;text-align:right;font-size:14px;';
					disclaimer.textContent = '本回答由 AI 生成，内容仅供参考，请仔细甄别';
					content.appendChild(disclaimer);
				} catch (e) {
					content.textContent = contentText || '渲染失败，请重试';
				}
				actions.innerHTML = `
			        <button id="aml-regenerate-btn" class="lform-size-middle button-transparent aml-confirm-btn-wrap" type="button" data-v-9f2998ba data-v-505b6a97>重新生成</button>
			        <button id="aml-result-confirm-btn" class="lform-size-middle button-transparent aml-confirm-btn-wrap" type="button" data-v-9f2998ba data-v-505b6a97>确定</button>
			    `;
				const regenBtn = document.getElementById('aml-regenerate-btn');
				setupLongPress(regenBtn, () => {
					clearAICacheForCurrentPath();
					renderLoadingUI();
					startAnalysis();
				}, 5);
				document.getElementById('aml-result-confirm-btn').addEventListener('click', hideModal);
			}
			async function startAnalysis() {
				if (isAnalyzing) return;
				const apiUrl = currentAMLSettings.aiProblemAnalysisApiUrl;
				const apiKey = currentAMLSettings.aiProblemAnalysisApiKey;
				const modelName = currentAMLSettings.aiProblemAnalysisModel || "gpt-3.5-turbo";
				if (!apiUrl || !apiKey) {
					await Swal.fire({ title: "配置错误", html: "请在设置中配置 AI API URL 和密钥", icon: "error" });
					return;
				}
				const problemText = getProblemText();
				if (!problemText) {
					await Swal.fire({ title: "无法获取题目信息", html: "请刷新页面重试", icon: "error" });
					return;
				}
				renderLoadingUI();
				isAnalyzing = true;
				const contentDiv = document.getElementById('aml-ai-content');
				let fullContent = '';
				let buffer = '';
				let dirty = false;
				...
				const scheduleRender = () => {
					...
					requestAnimationFrame(() => { requestAnimationFrame(renderIncremental); });
				};
				let isFirstChunk = true;
				const finishRendering = (finalContent) => {
					isAnalyzing = false;
					if (finalContent && finalContent.trim()) {
						dirty = true;
						renderScheduled = false;
						renderIncremental();
						setAICache(location.pathname, finalContent);
						renderResultUI(finalContent);
					} else {
						contentDiv.innerHTML = '<div style="color:#e74c3c;">未获取到有效内容，请检查 API 配置或重试</div>';
						const actions = document.getElementById('aml-ai-actions');
						actions.innerHTML = `<button id="aml-result-confirm-btn" class="lform-size-middle button-transparent aml-confirm-btn-wrap" type="button" data-v-9f2998ba data-v-505b6a97>确定</button>`;
						document.getElementById('aml-result-confirm-btn').addEventListener('click', hideModal);
					}
				};
				const controller = new AbortController();
				console.debug('[AML-AI-Stream]', 'A) Request start', { url: apiUrl, model: modelName, problemTextLength: problemText.length });
				const timeoutId = setTimeout(() => controller.abort(), 60000);
				const processChunk = async (chunkText) => {
					buffer += chunkText;
					const lines = buffer.split('\n');
					console.debug('[AML-AI-Stream]', 'D) Buffer state', { bufferLen: buffer.length, lineCount: lines.length });
					buffer = lines.pop() || '';
					for (const line of lines) {
						const trimmed = line.trim();
						if (!trimmed) continue;
						if (trimmed.startsWith('data: ')) {
							console.debug('[AML-AI-Stream]', 'E) Raw data line', trimmed.substring(0, 100));
							const data = trimmed.slice(6);
							if (data === '[DONE]') {
								clearTimeout(timeoutId);
								console.debug('[AML-AI-Stream]', 'G) Completion [DONE]', { fullContentLen: fullContent.length, done: true });
								finishRendering(fullContent);
								return true;
							}
							try {
								...
								if (delta !== undefined && delta !== null) {
									if (delta === '') {
										console.debug('[AML-AI-Type]', 'blank delta');
									} else {
										console.debug('[AML-AI-Stream]', 'F) Delta received', delta.substring(0, 50));
										if (isFirstChunk) {
				...
				GM_fetch(apiUrl, {
					...
					body: JSON.stringify({
						...
						messages: [
							...
							{ role: "user", content: getUserPrompt(problemText) }
						],
						stream: true,
					}),
					...
					console.debug('[AML-AI-Stream]', 'B) Response received', { status: response.status, contentType: response.headers.get('content-type'), bodyNotNull: response.body != null });
					const reader = response.body.getReader();
					const decoder = new TextDecoder();
					let done = false;
					while (!done) {
						const { value, done: readerDone } = await reader.read();
						console.debug('[AML-AI-Stream]', 'C) Chunk read', { done: readerDone, byteLength: value?.byteLength });
						done = readerDone;
						if (value) {
							const chunkText = decoder.decode(value, { stream: !done });
							const finished = await processChunk(chunkText);
							if (finished) return;
						}
					}
					clearTimeout(timeoutId);
					if (fullContent) {
						if (!document.querySelector('#aml-ai-actions #aml-result-confirm-btn')) {
							console.debug('[AML-AI-Stream]', 'G) Completion stream end', { fullContentLen: fullContent.length, done: true });
							finishRendering(fullContent);
						}
						return;
					}
					console.debug('[AML-AI-Stream]', 'G) Completion empty', { fullContentLen: 0, done: true });
					finishRendering('');
				}).catch(async (err) => {
					console.debug('[AML-AI-Stream]', 'H) Error caught', { name: err?.name, message: err?.message, stack: err?.stack?.substring(0, 100) });
					clearTimeout(timeoutId);
					if (err.name === 'AbortError') {
						isAnalyzing = false;
						contentDiv.innerHTML = '请求超时，请稍后重试';
						const actions = document.getElementById('aml-ai-actions');
						actions.innerHTML = `<button id="aml-result-confirm-btn" class="lform-size-middle button-transparent aml-confirm-btn-wrap" type="button" data-v-505b6a97>确定</button>`;
						document.getElementById('aml-result-confirm-btn').addEventListener('click', hideModal);
						return;
					}
					if (err.response) {
						try {
							...
							try {
								...
								if (content) {
									...
									console.debug('[AML-AI-Stream]', 'G) Completion catch JSON', { fullContentLen: fullContent.length, done: true });
									finishRendering(fullContent);
									return;
								}
							} catch (e) {
								...
								if (allContent) {
									...
									console.debug('[AML-AI-Stream]', 'G) Completion catch lines', { fullContentLen: fullContent.length, done: true });
									finishRendering(fullContent);
									return;
								}
							}
					...
					isAnalyzing = false;
					contentDiv.innerHTML = '请求出错，请检查网络或 API 配置';
					const actions = document.getElementById('aml-ai-actions');
					actions.innerHTML = `<button id="aml-result-confirm-btn" class="lform-size-middle button-transparent aml-confirm-btn-wrap" type="button" data-v-505b6a97>确定</button>`;
					document.getElementById('aml-result-confirm-btn').addEventListener('click', hideModal);
				});
			...
			function openAIModal() {
				...
				const cached = getAICacheForCurrentPath();
				if (cached && cached.content) {
					renderResultUI(cached.content);
				} else {
					renderConfirmUI();
				}
			}
			const handleAIAnalyzeClick = async (event) => {
			...
			const addAIAnalysisButtonToActions = () => {
				const btnActions = document.querySelector(".btn-actions");
				if (!btnActions) {
					setTimeout(addAIAnalysisButtonToActions, 500);
					return;
				}
				if (btnActions.querySelector(".aml-ai-analyze-btn") || !shouldShowAIAnalysisButton()) return;
				const aiBtn = document.createElement("button");
				aiBtn.className = "lform-size-middle button-transparent aml-ai-analyze-btn";
				aiBtn.type = "button";
				aiBtn.innerHTML = "AI 分析";
				aiBtn.setAttribute('data-v-505b6a97', '');
				addManagedEventListener(aiBtn, "click", handleAIAnalyzeClick);
				btnActions.appendChild(aiBtn);
				if (!window._amlObserver) {
					const observer = new MutationObserver(() => {
						if (!btnActions.querySelector(".aml-ai-analyze-btn") && shouldShowAIAnalysisButton()) {
							const newBtn = aiBtn.cloneNode(true);
							addManagedEventListener(newBtn, "click", handleAIAnalyzeClick);
							btnActions.appendChild(newBtn);
						}
					});
					observer.observe(btnActions, { childList: true });
					window._amlObserver = observer;
				}
			};
			const addAIAnalysisButtonToIdeToolbar = () => {
				const actions = document.querySelector(".ide-toolbar .actions");
				if (!actions) {
					setTimeout(addAIAnalysisButtonToIdeToolbar, 500);
					return;
				}
				if (actions.querySelector(".aml-ai-analyze-btn") || !shouldShowAIAnalysisButton()) return;
				const submitLink = actions.querySelector("a.title");
				if (!submitLink) return;
				const aiBtn = submitLink.cloneNode(true);
				const textSpan = aiBtn.querySelector('.text');
				if (textSpan) textSpan.textContent = 'AI 分析';
				const iconContainer = aiBtn.querySelector('.icon');
				if (iconContainer) {
					iconContainer.innerHTML = `
			            <svg class="svg-inline--fa fa-robot" viewBox="0 0 640 512">
			                <path fill="currentColor" d="M352 0c0-17.7-14.3-32-32-32S288-17.7 288 0l0 64-96 0c-53 0-96 43-96 96l0 224c0 53 43 96 96 96l256 0c53 0 96-43 96-96l0-224c0-53-43-96-96-96l-96 0 0-64zM160 368c0-13.3 10.7-24 24-24l32 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-32 0c-13.3 0-24-10.7-24-24zm120 0c0-13.3 10.7-24 24-24l32 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-32 0c-13.3 0-24-10.7-24-24zm120 0c0-13.3 10.7-24 24-24l32 0c13.3 0 24 10.7 24 24s-10.7 24-24 24l-32 0c-13.3 0-24-10.7-24-24zM224 176a48 48 0 1 1 0 96 48 48 0 1 1 0-96zm144 48a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zM64 224c0-17.7-14.3-32-32-32S0 206.3 0 224l0 96c0 17.7 14.3 32 32 32s32-14.3 32-32l0-96zm544-32c-17.7 0-32 14.3-32 32l0 96c0 17.7 14.3 32 32 32s32-14.3 32-32l0-96c0-17.7-14.3-32-32-32z"></path>
			            </svg>
			        `;
				}
				aiBtn.classList.add('aml-ai-analyze-btn');
				submitLink.parentNode.insertBefore(aiBtn, submitLink.nextSibling);
				addManagedEventListener(aiBtn, "click", handleAIAnalyzeClick);
			};
			if (shouldShowAIAnalysisButton()) {
				try {
					if (!document.getElementById('aml-ai-anim-style')) {
						const style = document.createElement('style');
						style.id = 'aml-ai-anim-style';
						style.textContent = `
			                .aml-spinner { display: inline-block; width: 20px; height: 20px; border: 3px solid #e2e8f0; border-top-color: #6366f1; border-radius: 50%; animation: aml-spin 0.8s linear infinite; }
			                @keyframes aml-spin { to { transform: rotate(360deg); } }
			            `;
						document.head.appendChild(style);
					}
					if (!document.getElementById('aml-ai-progress-style')) {
						const style = document.createElement('style');
						style.id = 'aml-ai-progress-style';
						style.textContent = `
			                .ai-action-progress {
			    				position: absolute;
			    				top: 0; right: 0; bottom: 0; left: 0;
			    				width: 0%;
			    				background: rgba(var(--lcolor--primary, 0, 123, 255), 0.25);
			    				transition: width 0.2s ease, opacity 0.2s ease;
			    				z-index: 0;
			    				border-radius: 0;
			    				pointer-events: none;
							}
			                .aml-confirm-btn-wrap { position:relative; overflow:hidden; }
			                .aml-confirm-btn-text { position:relative; z-index:1; }
			            `;
						document.head.appendChild(style);
					}
					let modal = document.getElementById('aml-ai-modal');
					if (!modal) {
						modal = document.createElement('div');
						modal.id = 'aml-ai-modal';
						modal.className = 'aml-modal modal hide';
						modal.setAttribute('data-v-cd5b5a1b', '');
						modal.style = 'overflow: hidden;';
						modal.innerHTML = `
			                <div data-v-cd5b5a1b class="background"></div>
			                <div data-v-176b97b3="" data-v-0910ec7f="" class="l-card container type-burger" style="--l-modal--min-width: 560px;font-family:'LXGW Wenkai Screen'">
			                    <div data-v-0910ec7f="">
			                        <h3 data-v-81eb186a="" class="lfe-h3" id="aml-modal-title">AI 题目分析</h3>
			                        <div data-v-0910ec7f="" class="close" id="aml-modal-close" style="cursor:pointer;position:absolute;right:16px;top:16px;">
			                    ...
			                    <div data-v-0910ec7f="">
			                        ...
			                        <div data-v-02424d02="" class="l-form-layout row" id="aml-ai-actions" style="justify-content:flex-end;margin-top:10px;gap:10px;display:flex;flex-wrap:wrap;">
			                        </div>
			                    </div>
			                </div>
			            `;
						document.body.appendChild(modal);
						document.getElementById('aml-modal-close').addEventListener('click', hideModal);
						modal.querySelector('.background').addEventListener('click', hideModal);
					}
					const addAllButtons = () => {
						addAIAnalysisButtonToActions();
						addAIAnalysisButtonToIdeToolbar();
					};
					if (document.readyState === "loading") {
						addManagedEventListener(document, "DOMContentLoaded", addAllButtons);
					} else {
				...
				document.querySelectorAll(".aml-ai-analyze-btn").forEach(el => el.remove());
				if (window._amlObserver) {
					window._amlObserver.disconnect();
					delete window._amlObserver;
				}
				const delAIModal = document.getElementById('aml-ai-modal');
				if (delAIModal) delAIModal.remove();
			}
    ...
    color: rgb(var(--l-button--real-color));" class="lform-size-small" type="button" data-type="render" alt="渲染为 HTML，请小心鉴别">渲染为 HTML</button>
							`.trim();
								...
								addManagedEventListener(copyBtn, "click", async function () {
									GM_setClipboard(it);
									await Swal.fire({
										title: "复制成功",
										text: "用户信息已复制到剪贴板！",
					...
					GM_xmlhttpRequest({
						...
						onload: function (response) {
							try {
								...
								console.debug(newslogan);
								try {
									console.debug("checking 2");
									if (
			...
			if (currentPath.startsWith("/user/mine")) {
				try {
					...
					const apiUrl = `${SUPABASE_URL}/rest/v1/user_status?uid=eq.${uid}&select=*`;
					console.debug(apiUrl, uid, SUPABASE_ANON_KEY);
					GM_xmlhttpRequest({
						method: "GET",
						url: apiUrl,
						headers: {
		...
		`);
					...
					async function rrenderMarkdown(element) {
						...
						if (Date.now() - lastUpdate > 31 * 24 * 60 * 60 * 1000) {
							...
							GM_xmlhttpRequest({
								...
								onload: function (response) {
									...
									console.debug("返回数据：", response.responseText);
									try {
										const data = JSON.parse(response.responseText);
										...
										console.debug("解析后的JSON数据：", data);
									} catch (e) {
										console.debug("数据不是标准JSON格式", e);
									}
								},
								onerror: function (error) {
									console.error("❌ 请求失败：", error);
			...
			if (currentAMLSettings.problemColors) {
				try {
					console.debug(Date.now(), "Starting to set up problem color map...");
			...
			if (currentAMLSettings.walineEnabled && window.location.pathname == '/') {
				try {
					...
					console.debug('[Waline Home] 错误:', e);
				}
			}
			if (currentAMLSettings.memoEnabled && window.location.pathname === "/") {
				...
				`);
					async function renderMemoContent(content) {
					...
					if (userId && !chatWSRD) {
						...
						try {
							...
							ws.onopen = () => {
								...
								console.debug("[Chat] 已加入聊天频道");
							};
							ws.onmessage = (event) => {
								console.debug("[Chat] 收到消息:", event);
...
#qq-emoji-search-input:focus {
  ...
  outline: none !important;
			...
			async function checkScriptVersion(op) {
				...
				for (const result of results) {
					if (result.priority) {
						...
						console.debug(`优先源 ${result.sourceName} 的版本检查结果：`, result);
					}
				}
				console.debug(latestVersion, downloadUrl, sourceUsed);
				if (!latestVersion) {
					return;
				}
	...
	const SUPABASE_ANON_KEY =
		"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt0d2h3dmFmeXd3ZWtma3Zza2JrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg2OTU1NDgsImV4cCI6MjA4NDI3MTU0OH0.tSCRz7ENeCT3NXt891equmSBfW_UsXHUdKSVMoxveKQ";
	console.debug("当前用户 ID:", getCurrentUserId());
	const uid = getCurrentUserId();
	if (uid) {
		try {
			...
			async function initOnlineModule() {
				...
				if (storedEmail && storedPassword) {
					if (storedEmail.startsWith(uid + "_")) {
						...
						console.debug("用户" + uid + "邮箱不匹配，清除旧数据");
						GM_setValue("amlgEmail_" + uid, "");
						GM_setValue("amlgPassword_" + uid, "");
						GM_setValue("SlogenDeleted_" + uid, false);
					}
				} else {
					console.debug("用户" + uid + "未注册，注册提示已在页面加载时显示");
				}
			}
			initOnlineModule();
		} catch (e) {
...
async function reportActive(uid) {
	try {
		...
		if (GM_getValue("showregisterprompt", 0) <= (new Date()).getTime() &&
			...
			console.debug("凭据不匹配，清除旧数据并重新授权");
			GM_setValue("amlgEmail_" + uid, "");
			GM_setValue("amlgPassword_" + uid, "");
			GM_setValue("SlogenDeleted_" + uid, false);
			if (heartbeatInterval) clearInterval(heartbeatInterval);
...
function checkUpdate(uid) {
	if (!currentAMLSettings.slogenTimeEnabled) {
		...
		const storedPassword = GM_getValue("amlgPassword_" + uid, "");
		if (storedEmail && storedPassword) {
			deleteSlogenData(uid).then(() => {
				GM_setValue("amlgDate_" + uid, 0);
				console.debug("用户" + uid + "已关闭slogenTimeEnabled，已删除上传数据");
			}).catch((e) => {
				console.warn("用户" + uid + "删除数据失败:", e);
			});
		}
		return;
	}
	...
	if (
		...
		console.debug("User " + uid + " updated.");
		return;
	}
}
...
async function register(uid) {
	...
	try {
		...
		if (result.success) {
			...
			console.debug("[AML Register] 用户" + uid + "注册失败，服务器返回失败: " + (result.message || "未知原因"));
		}
	} catch (e) {
		...
		if (await register(uid)) {
			...
			if (currentAMLSettings.slogenTimeEnabled) {
				...
				try {
					await deleteSlogenData(uid);
					GM_setValue("amlgDate_" + uid, 0);
					console.debug("用户" + uid + "已关闭slogenTimeEnabled，注册后已删除数据");
				} catch (e) {
					console.warn("用户" + uid + "注册后删除数据失败:", e);
				}
			}
		} else {
			loading.close();
			const errorMsg = result.message || "请检查 CP OAuth 账号是否绑定了正确的洛谷账号后重试";
		...
		try {
			...
			if ((!existingEmail || !existingEmail.startsWith(uid + "_")) && GM_getValue("showregisterprompt", 0) <= (new Date()).getTime()) {
				if (existingEmail && !existingEmail.startsWith(uid + "_")) {
					console.debug("用户" + uid + "邮箱不匹配，需要重新注册");
				}
				await showRegisterPrompt(uid);
			}
...
function patchXHR(eventName = "luogu-xhr-intercept") {
	...
	console.debug(`✅ XHR 拦截已启动，事件名：${eventName}`);
}
patchXHR();
window.addEventListener("luogu-xhr-intercept", (e) => {
	...
	if (data && data.instance && data.template && data.status) {
		...
		if (target) {
			console.debug("✅ 找到 lentille-context，开始替换内容");
			target.innerHTML = JSON.stringify(data, null, 2);
			console.debug("✅ 替换完成，新的内容：", target.innerHTML);
		}
	}
});
function replacerss() {
	...
	if (currentAMLSettings.codeFolding) {
		const coptBtns = document.querySelectorAll('.codecopy-btn');
		coptBtns.forEach(btn => {
			btn.remove();
		});
	}
}
